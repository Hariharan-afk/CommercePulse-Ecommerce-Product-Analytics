...
session_steps AS (
    ...
    GROUP BY event_month, user_session
),
...
SELECT
    ...
    ROUND(
        100.0 * e.cart_events
        / NULLIF(e.view_events, 0),
        2
    ) AS event_view_to_cart_rate_pct,

    ROUND(
        100.0 * e.purchase_events
        / NULLIF(e.view_events, 0),
        2
    ) AS event_view_to_purchase_rate_pct

FROM event_monthly e
LEFT JOIN funnel_monthly f
